...
public class TransactionService {
	...
	public void deposit(BankAccount bankAccount, double amount) throws Exception{
		try {
			...
			throw new Exception("Deposit failed for account " + bankAccount.getAccountNumber() + " with amount " + amount, e);
		}finally {
	...
	public void withdraw(BankAccount bankAccount, double amount) throws Exception{
		try{
			...
			throw new Exception("withdraw failed for account " + bankAccount.getAccountNumber() + " with amount " + amount, e);
		} finally {
			bankAccount.getLock().unlock();
		}
	}
	...
	public double useCheckCard(CheckingAccount bankAccount, double amount) throws Exception{
		try{
			...
			throw new Exception("useCheckCard failed for account " + bankAccount.getAccountNumber() + " with amount " + amount, e);
		} finally {
			bankAccount.getLock().unlock();
		}
	...
	public double applyInterest(SavingsAccount bankAccount) throws Exception {
		...
		try{
			...
			throw new Exception("applyInterest failed for account " + bankAccount.getAccountNumber() + " with interest " + interest, e);
		} finally {
			bankAccount.getLock().unlock();
		}
		return interest;
